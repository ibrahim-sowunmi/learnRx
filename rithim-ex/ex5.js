...
const listNames = songs => songs.forEach(song => print(song.name));

const listSongDetails = songs => songs.forEach(song => print(`${song.name}, by ${song.artist} (${song.duration})`));

const summerJamCountOne = songs => {
  var count = 0;
  const months = [6, 7, 8];
  
  songs.forEach(song => months.includes(song.month) ? count++ : "");
  return count;
};

const getDurations = songs => songs.map(song => song.duration);

const getDurationInSeconds = songs => songs.map(song => {
  const timeArr = song.duration.split(":");
  var seconds = 0;
  for (let i = 0; i < timeArr[0]; i++) {
    seconds += 60;
  }
...
const getMainArtists = songs => songs.map(song => song.artist.split(" featuring")[0]);

const getBigHits = songs => songs.filter(song => song.weeksAtNumberOne > 10)

const getShortSongs = songs => songs.filter(song => song.duration.split(":")[0] < 3)
 
const getSongsByArtist = (songs, artist) => songs.filter(song => song.artist == artist)

const summerJamCount = songs => songs.reduce((acc, curr) => {
  const months = [6, 7, 8];
  if (months.includes(curr.month)) acc++;
  return acc;
}, 0)

const getTotalDurationInSeconds = songs => songs.reduce((acc, curr) => {
  const timeArr = curr.duration.split(":");
  for (let i = 0; i < timeArr[0]; i++) {
    acc += 60;
  }
  acc += parseInt(timeArr[1]);
  return acc;
}, 0)

const getSongCountByArtist = songs => songs.reduce((acc, curr) => {
  var artist = curr.artist.split(" featuring")[0];
  acc[artist] ? acc[artist]++ : acc[artist] = 1;
  console.log(acc)
  return acc;
}, {})

const averageWeeksAtNumberOne = songs => songs.reduce((acc, curr) => acc + parseInt(curr.weeksAtNumberOne), 0)/songs.length;
